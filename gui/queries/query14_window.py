...
class Query14Window(BaseQueryWindow):


    def __init__(self, query_dao: QueryDAO, parent=None):
        super().__init__(query_dao, parent)
        self.setWindowTitle("Запрос 14 – Сводка по сборке")
        self._load_filters()

    def _get_title(self) -> str:
        return "Запрос 14: Сводка по изделиям в сборке"
    ...
    def _load_filters(self):
        """Загружает списки цехов и участков."""
        try:
    ...
    def _execute_query(self):
        ...
        total, data = self.query_dao.query14_assembly_summary(
            shop_id=shop_id,
            section_id=section_id
        )

        columns = ["Цех", "Участок", "Категория", "Количество"]
        self.display_results(data, columns=columns)
        self.status_bar.showMessage(f"Всего изделий в сборке: {total}")
