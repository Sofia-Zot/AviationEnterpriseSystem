...
class Query3aWorkersWindow(BaseQueryWindow):


    def __init__(self, query_dao: QueryDAO, parent=None):
        super().__init__(query_dao, parent)
        self.setWindowTitle("Запрос 3a – Рабочие")
        self._load_filters()

    def _get_title(self) -> str:
        return "Запрос 3a: Кадровый состав (рабочие)"
    ...
    def _execute_query(self):
        ...
        data = self.query_dao.query3_workers(
            shop_id=shop_id,
            section_id=section_id
        )

        columns = ["ФИО", "Профессия", "Разряд", "Бригада", "Участок", "Цех"]
